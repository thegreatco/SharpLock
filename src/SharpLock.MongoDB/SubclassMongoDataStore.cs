...
namespace SharpLock.MongoDB
{
    public class MongoDataStore<TBaseObject, TLockableObject> : IDataStore<TBaseObject, TLockableObject, ObjectId>
        where TLockableObject : ISharpLockable<ObjectId> where TBaseObject : ISharpLockableBase<ObjectId>
    {
            ...
            CancellationToken token)
        {
            ...
            _logger.Trace("Acquire Lock Query: {Query}, Acquire Lock Update: {Update}",
        ...
        public Task<TBaseObject> RefreshLockAsync(ObjectId baseObjId, TLockableObject obj,
            Expression<Func<TBaseObject, IEnumerable<TLockableObject>>> fieldSelector, CancellationToken token)
        {
            var query = Builders<TBaseObject>.Filter.And(
                Builders<TBaseObject>.Filter.Eq(x => x.Id, baseObjId),
                Builders<TBaseObject>.Filter.ElemMatch(fieldSelector,
            ...
            _logger.Trace("Refresh Lock Query: {Query}, Refresh Lock Update: {Update}",
                query.Render(BsonSerializer.SerializerRegistry.GetSerializer<TBaseObject>(),
                    BsonSerializer.SerializerRegistry).ToJson(),
                update.Render(BsonSerializer.SerializerRegistry.GetSerializer<TBaseObject>(),
                    BsonSerializer.SerializerRegistry).ToJson());
        ...
        public Task<TBaseObject> RefreshLockAsync(ObjectId baseObjId, TLockableObject obj,
            Expression<Func<TBaseObject, TLockableObject>> fieldSelector, CancellationToken token)
        {
            var query = Builders<TBaseObject>.Filter.And(
                Builders<TBaseObject>.Filter.Eq(x => x.Id, baseObjId),
                Builders<TBaseObject>.Filter.Eq(Combine(fieldSelector, x => x.Id), obj.Id),
            ...
            var update = Builders<TBaseObject>.Update.Set(Combine(fieldSelector, x => x.UpdateLock),
                DateTime.UtcNow.Add(_lockTime));
        ...
        public Task<TBaseObject> ReleaseLockAsync(ObjectId baseObjId, TLockableObject obj,
            Expression<Func<TBaseObject, IEnumerable<TLockableObject>>> fieldSelector, CancellationToken token)
        {
            var query = Builders<TBaseObject>.Filter.And(
                Builders<TBaseObject>.Filter.Eq(x => x.Id, baseObjId),
